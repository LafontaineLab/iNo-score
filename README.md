...
This archive contains codes to sort the target gene by index of nucleolar disruption (iNo).
  ...
  - data should contain the dataset of images named as 
    X_Y_sZ_w2.TIF (for GFP image) and X_Y_sZ_w2.TIF for (DAPI image)
    (where:
      - X is the name of the plate, 
      - Y the name of the well, 
      - Z the index of the images/sites in a well
    ):
    - one directory by plate (the name of the directory is the name
                              of the plate)
  ...
  - src contains the source codes
    - Launch the "s_main" script in order to segment the nuclei, 
      compute the features and analyze them.
